...
for president, birthday in data:
    treedata.Insert('', president, president, values=[birthday]+[1,2,3], icon = check[0])

sg.theme('LightPurple')
sg.set_options(font=('Helvetica', 16))
layout = [
    [sg.Tree(data=treedata, headings=headings[1:], auto_size_columns=True,
             num_rows=10, col_widths=20, key='-TREE-', row_height=48, metadata=[],
...
tree = window['-TREE-']
tree.Widget.heading('#0', text=headings[0])
